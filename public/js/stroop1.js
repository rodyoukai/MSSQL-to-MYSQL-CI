...
function next() {
  ...
  $('#content').html(ensayo+"<br>"+arrayJS[ensayo][3]+"<br>"+errors.length+"<br>"+success.length);	
...
for (i=30;i<60;i++) {
  ...
  latencia_e1 = $.sum(e1)/e1.length;
  
  for (i=0;i<30;i++) {
		
		indice_interferencia[i] = e1[i] - neutros[i];
		
	} 
  
  
 indice_e1 =  $.sum(indice_interferencia)/indice_interferencia.length;
  ...
  $.post(base+"test/guardar_stroop/"+tipo+"/"+tematica+"/"+usuario,
       {
            ...
            interferencia_e1: indice_e1.toFixed(2),

            latencia_e1: latencia_e1.toFixed(2),
